...
be a directory). We never prefix
with the base-branch name, so `warp/<task>/slice-<id>` is always safe.

Pure Python so it runs identically on Windows/macOS/Linux (no bash-shim issues).
...
def _is_dirty(path: str) -> bool:
    #  a non-zero `git status` (corrupt/locked worktree, bad
...
def cmd_pin_base(args) -> int:
    ...
    Worktree base can be flaky: `Agent(isolation:worktree)` does NOT guarantee the
    ...
    orchestrator's committed DESIGN HEAD), so a slice could silently build against code
    that was missing the very work it depended on. The fix: pin the base explicitly.
    ...
    #    detached/clobbered HEAD - exactly the chaos an unguarded fan-out can hit.
...
def cmd_check(args) -> int:
    ...
    print("  → run: python workflows/warp/worktrees.py cleanup --task <slug> "
