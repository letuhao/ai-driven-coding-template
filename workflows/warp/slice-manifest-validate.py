...
workflow mode (see the /warp design): it converts
...
Usage:
  python workflows/warp/slice-manifest-validate.py <manifest.yaml|.json> [--json]
...
def overlaps(a: tuple[str, ...], b: tuple[str, ...]) -> bool:
    ...
    Comparison is CASE-INSENSITIVE : the project's target
...
def validate_manifest(manifest: dict) -> list[Finding]:
    ...
    for idx, sl in enumerate(slices):
        ...
        #  reject globs the overlap check can't reason about,
    ...
    if not isinstance(mp, dict):
        findings.append(Finding("WARN", "R4", "`merge_plan` missing - no integrate order / reconcile evidence"))
    else:
        ...
        if not str(mp.get("reconcile_evidence", "")).strip():
            findings.append(Finding(
                "WARN", "R4",
                "`reconcile_evidence` empty - define the cross-module smoke that "
...
def verify_frozen_shas(manifest: dict, blob_getter=_head_blob_sha) -> list[Finding]:
    ...
    immutability the slices depend on .
